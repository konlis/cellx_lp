 ...
 *      (e.g., your private inbox — NOT kontakt@cellx.pl unless you want that)
 * 5. Deploy to Vercel
 */
...
export default async function handler(req, res) {
  ...
  const allowedOrigin = process.env.ALLOWED_ORIGIN || 'https://homefit.cellx.pl';
  ...
  const { name, phone, email, city, pv_power, inverter, version, message, website } = req.body;
  ...
  if (String(name).length > 100 || String(phone).length > 30 || String(email).length > 254 ||
      ...
      (pv_power && String(pv_power).length > 20) || (inverter && String(inverter).length > 100) ||
      (version && String(version).length > 20)) {
    return res.status(400).json({ error: 'Field length exceeded' });
  }

  // Strict email validation (C1 fix)
  const emailRegex = /^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/;
  if (!emailRegex.test(email) || email.length > 254) {
    return res.status(400).json({ error: 'Invalid email format' });
  }

  // Phone format validation (M2 fix)
  ...
  // Sanitized subject line (C3 fix)
  const subject = `Nowe zapytanie Home Fit+ — ${sanitizePlainText(name)} z ${sanitizePlainText(city || 'nieznane miasto')}`;

  const htmlBody = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <h2 style="color: #1d1d1f; border-bottom: 3px solid #7ed957; padding-bottom: 10px;">
        Nowe zapytanie — CellX Home Fit+
      </h2>
      <table style="width: 100%; border-collapse: collapse; margin-top: 16px;">
        ...
        <tr style="background: #f9f9f9;"><td style="padding: 8px 12px; font-weight: bold; color: #555;">Miasto:</td><td style="padding: 8px 12px;">${escapeHtml(city || '—')}</td></tr>
        <tr><td style="padding: 8px 12px; font-weight: bold; color: #555;">Moc PV (kWp):</td><td style="padding: 8px 12px;">${escapeHtml(pv_power || '—')}</td></tr>
        <tr style="background: #f9f9f9;"><td style="padding: 8px 12px; font-weight: bold; color: #555;">Model falownika:</td><td style="padding: 8px 12px;">${escapeHtml(inverter || '—')}</td></tr>
        <tr><td style="padding: 8px 12px; font-weight: bold; color: #555;">Wersja:</td><td style="padding: 8px 12px;">${escapeHtml(version || 'Nie wiem')}</td></tr>
      </table>
      ${message ? `
      ...
      <p style="margin-top: 24px; font-size: 12px; color: #999;">
        Wiadomość wysłana z formularza na homefit.cellx.pl
      </p>
    </div>
  `;
